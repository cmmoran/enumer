...
var goldenSQLInt = []Golden{
	{"primeSqlInt", primeSqlIn},
}
...
var goldenJSONAndSQLInt = []Golden{
	{"primeJsonAndSqlInt", primeJsonAndSqlIn},
}
...
var goldenTrimSuffix = []Golden{
	{"trimSuffix", trimSuffixIn},
}
...
const trimSuffixIn = `type Day int
const (
	MondayDay Day = iota
	TuesdayDay
	WednesdayDay
...
const statusWithLinecommentAliasIn = `type Status int
const (
...
func TestGolden(t *testing.T) {
	...
	for _, test := range goldenSQLInt {
		runGoldenTest(t, test, false, false, true, false, false, false, false, true, "", "", "", "")
	}
	for _, test := range goldenJSONAndSQL {
		runGoldenTest(t, test, true, false, true, false, false, false, false, false, "", "", "", "")
	}
	for _, test := range goldenJSONAndSQLInt {
		runGoldenTest(t, test, true, false, true, false, false, false, false, true, "", "", "", "")
	}
	...
	t.Logf("trimSuffix")
	for _, test := range goldenTrimSuffix {
		runGoldenTest(t, test, false, false, false, false, false, false, false, false, "", "Day", "", "")
	}
...
func runGoldenTest(t *testing.T, test Golden, generateJSON, generateYAML, generateSQL, generateText, linecomment, generateGQLGen, generateValuesMethod, sqlint bool, trimPrefix, trimSuffix, prefix, suffix string) {
	...
	g.generate(tokens[1], generateJSON, generateYAML, generateSQL, generateText, generateGQLGen, sqlint, "noop", trimPrefix, trimSuffix, prefix, suffix, linecomment, generateValuesMethod)
	got := string(g.format())
